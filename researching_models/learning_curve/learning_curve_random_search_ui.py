...
class LearningCurveRandomSearchTab(QWidget):
    ...
    def on_analyze(self):
        try:
            ...
            logger.info(f"Параметры передачи: final_val={lc_result['final_val']:.4f}, final_test={lc_result['final_test']:.4f}, gap={lc_result['gap']:.4f}")
            ...
            try:
                # Удаляем model_name из lc_result чтобы избежать дублирования
