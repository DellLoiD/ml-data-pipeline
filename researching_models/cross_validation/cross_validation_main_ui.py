...
from researching_models.cross_validation.cross_validation_optuna_logic import OptunaAnalyzer
...
class CrossValidationMainUI(QWidget):
    def __init__(self):
        super().__init__()
    ...
    def init_ui(self):
        self.setWindowTitle("Кросс-валидация - Сравнение методов")
        main_layout = QVBoxLayout()

        title_label = QLabel("Подбор гиперпараметров и кросс-валидация")
        ...
        # Вкладка Optuna
        from .cross_validation_optuna_ui import CrossValidationOptunaTab
        self.optuna_tab = CrossValidationOptunaTab(analyzer=self.optuna_analyzer, main_window=self)
        self.tab_widget.addTab(self.optuna_tab, "Optuna")

        # Вкладка Random Search
        # from .cross_validation_random_search_ui import CrossValidationRandomSearchTab
        # self.random_search_tab = CrossValidationRandomSearchTab(analyzer=self.random_search_analyzer, main_window=self)
        # self.tab_widget.addTab(self.random_search_tab, "Random Search")
        main_layout.addWidget(self.tab_widget)

        # Удаляем проверку загрузки датасета в CrossValidationOptunaTab
...
# Удалена функция on_model_changed, так как она больше не нужна после удаления update_models
